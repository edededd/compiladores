...
int ImpCodeGen::visit(DoWhileStatement* s) {
    string l1 = next_label();  // Etiqueta de inicio del bucle
    string l2 = next_label();  // Etiqueta de salida del bucle

    codegen(l1, "skip");  // Marca el inicio del bucle
    ...
    // Salta al inicio del bucle si la condición es verdadera
    codegen(nolabel, "jmpz", l2);
    codegen(nolabel, "goto", l1);

    codegen(l2, "skip");  // Marca el final del bucle
...
int ImpCodeGen::visit(ForStatement* s) {
  ...
  s->e2->accept(this);
  codegen(loop_start, "load", direcciones.lookup(s->id));
  codegen(nolabel, "sub");
  codegen(nolabel, "push", 0);
  codegen(nolabel, "lt");

  // Jump to loop end if endExp <= startExp
  codegen(nolabel, "jmpz", l2);
  ...
  codegen(nolabel, "add");
  codegen(loop_start, "store", direcciones.lookup(s->id));
